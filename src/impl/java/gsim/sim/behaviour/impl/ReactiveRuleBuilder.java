...
public class ReactiveRuleBuilder {
            ...
            if (rule.isActivated()) {
                for (int i = 0; i < consequences.length; i++) {
                    ...
                    if (params != null && params.length > 0) {
                        ...
                        nRule += " =>\n";
                        nRule += " (bind ?res (call gsim.sim.behaviour.SimAction valueOf \"" + c.getClassName() + "\" \"" + ns + "\") )\n";
                        nRule += " (bind ?ctx (new gsim.sim.behaviour.Context))\n";
    ...
    private String buildUserRules_PerActionName_P(String ruleIdent, String nRule, String[] objParams, gsim.def.objects.behaviour.Action consequence,
        ...
        try {
            ...
            String ns = agent.getNameSpace().split("/")[0];
            nRule += " =>\n";
            nRule += " (bind ?res (call gsim.sim.behaviour.SimAction valueOf \"" + consequence.getClassName() + "\" \"" + ns + "\") )\n";
            nRule += " (bind ?ctx (new gsim.sim.behaviour.Context))\n";
            nRule += " (set ?ctx executionContext (?*agent* getExecutionContext(new java.lang.String \"" + role + "\")))\n";
            // nRule += " (set ?ctx executionContext (?*agent* getExecutionContext(new java.lang.String \"" + role + "\")))\n";
            nRule += ctxString;
