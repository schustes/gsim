...
public class Expand0 extends DynamicRuleBuilder implements java.io.Serializable {
     ...
     * @param toExpand
     *            - the element (part) of the state reprentation selected as new element in the next, new state
     * @param stateElemsWithoutExpansion
     *            - list of all elements belonging to the sate
     * @param context
     * @return
     * @throws JessException
     */
    public boolean createNextStatesCat(RuntimeAgent agent, Fact stateFact, Fact toExpand, ArrayList<Fact> allElemsList, Context context, boolean copy)
            throws JessException {
        ...
        try {
            ...
            StateFactElemCategorySpec unexpandedElemsSpec = extractCategoryElemSpec(allElemsList, context, toExpandParamName);// contains the fillers
                                                                                                                              // of
                                                                                                                              // other state-elems
            ...
            Fact newStateFactSplit_1 = expandStateDescription(stateFact, rootRule0, unexpandedElemsSpec.attributeSpec, context, oldDepth + 1, copy);
            String stateNameExpanded_New = newStateFactSplit_1.getSlotValue("name").stringValue(context);

            // the extracted value (=1 elem)
            Fact stateFactElem_1 = super.addStateFactCategoryElem(newStateFactSplit_1, toExpand.getSlotValue("param-name").stringValue(context), cat1,
                    context);
            ...
            Fact newStateFactSplit_2 = expandStateDescription(stateFact, rootRule0, unexpandedElemsSpec.attributeSpec, context, oldDepth + 1, copy);
            ...
            for (Fact s : unexpandedElemsSpec.facts) {
                ...
                Fact stateFactElem_2 = super.addStateFactCategoryElem(newStateFactSplit_2, toExpand.getSlotValue("param-name").stringValue(context),
                        cat2, context);
            ...
            if (r0.hasSelectors()) {
                context.getEngine().executeCommand(createNewSelectionNodesCat(b, r0, stateNameExpanded_New, unexpandedElemsSpec.attributeSpec,
                        fillersOfExpandAttribute, remaining, oldDepth + 1, context));
                context.getEngine().executeCommand(createNewSelectionNodesCat(b, r0, stateNameExpanded_Siblings, unexpandedElemsSpec.attributeSpec,
                        fillersOfSiblingAttributes, remaining, oldDepth + 1, context));
            } else {
    ...
    /*
     * //the same as createNextStates(), but without splitting - simply add category, delete old rules, and create new ones up to root. private void
     * recursivelyAddNewAttribute(Context context, Fact addedCategory) {
     * 
     ...
     * stateFact.getSlotValue("parent").stringValue(context); Fact parentFact = FactHandler.getInstance().getFact("state-fact", stateFactParent,
     * context);
     * 
     ...
     * StateFactElemCategorySpec unexpandedElemsSpec = extractCategoryElemSpec( allElemsList, context, toExpandParamName);//contains the fillers of
     * other state-elems //for the same parameter
     * 
     * if (unexpandedElemsSpec.facts.size() == 0) { Logger.getLogger(Rete.class).debug( " Statefact " + stateFactName + " is now fully expanded.");
     * return false; } //this means that there are no elems of this state with a value for this //parameter --> the selected fact is the last, and
     * because there is only //one value, nothing can be expanded
     * 
     * int oldDepth = (int) stateFact.getSlotValue("depth").floatValue( context); // ----------------------------------------------// //now: create
     * two states: one with the elems for the selected param, //one for ?
     * 
     * TreeExpansionBuilder b = new TreeExpansionBuilder(agent); // ceate and insert extracted new category (state fact, and // elem-fact) // String
     * toExpandAttributeSpec = // toExpand.getSlotValue("param-name").stringValue(context); String[] fillersOfExpandAttribute = new String[] {
     * toExpandCategoryValue }; String[] fillersOfSiblingAttributes = unexpandedElemsSpec.fillers .toArray(new
     ...
     * 
     * context.getEngine().assertFact(stateFactElem_1); // // insert the - constant - remaining rest of the original state //??? is the same???
     * 
     * //the result: one state with one elem (the extracted), this state with the //rest (e.g.: state1=a, state2=b or c. Old state a or b or c)
     * //expand=create new Fact from parent 'stateFact' (new name is generated) Fact newStateFactSplit_2 = expandStateDescription(stateFact,
     * unexpandedElemsSpec.attributeSpec, context, oldDepth + 1); //this is the generated name: String stateNameExpanded_Siblings =
     * newStateFactSplit_2 .getSlotValue("name").stringValue(context);
     * 
     * //the elems holding the filler values for the remaining categories of //the state for (Fact s : unexpandedElemsSpec.facts) { String cat2 =
     * s.getSlotValue("category").stringValue(context); //as spec holds the rest, stateFactElem_2 holds the elements disjunct from //the parameter
     * value specified in stateFactElem_1. The union is the set //of parameter values defined by the parent state. Fact stateFactElem_2 =
     * addStateFactCategoryElem(newStateFactSplit_2, cat2, context); context.getEngine().assertFact(stateFactElem_2); } // original rule String
     * rootRule0 = stateNameExpanded_New.split("_")[0]; RLRule r0 = agent.getBehaviour().getRLRule(rootRule0); // // remaining list holds the original
     * list of sf-elems except the // to-expand-elem Fact[] remaining = new Fact[allElemsList.size()]; allElemsList.toArray(remaining); if
     * (r0.hasSelectors()) { context.getEngine().executeCommand( this.createNewSelectionNodesCat(b, r0, stateNameExpanded_New,
     * unexpandedElemsSpec.attributeSpec, fillersOfExpandAttribute, remaining, oldDepth + 1, context)); context.getEngine().executeCommand(
     * this.createNewSelectionNodesCat(b, r0, stateNameExpanded_Siblings, unexpandedElemsSpec.attributeSpec, fillersOfSiblingAttributes, remaining,
     * oldDepth + 1, context)); } else { context.getEngine().executeCommand( this.createNewExperimentalRuleCat(b, r0, stateNameExpanded_New,
     * unexpandedElemsSpec.attributeSpec,//--> List with all elems of parent plus added child-statefact. fillersOfExpandAttribute, remaining,
     * context)); context.getEngine() .executeCommand( this.createNewExperimentalRuleCat(b, r0, stateNameExpanded_Siblings,
     * unexpandedElemsSpec.attributeSpec, fillersOfSiblingAttributes, remaining, context)); } insertNewActionNodes(context, stateNameExpanded_New);
     * 
     ...
     */
    public void createNextStatesNum(RuntimeAgent agent, Fact stateFact, Fact toExpand, ArrayList<Fact> allElems, Context context, boolean copy)
            throws JessException {
        ...
        try {
            ...
            if (rootRule.hasSelectors()) {
                context.getEngine()
                        .executeCommand(createNewSelectionNodes(b, rootRule, agent, newStateName1, stateFactElem_split1, remaining, context));
                context.getEngine()
    ...
    private void appendRemainingStateFactElemsInterval(Fact[] remaining, String stateFactName, Context context) throws JessException {
        for (Fact f : remaining) {
            ...
            FactHandler.getInstance().addStateFactElement(context.getEngine(), stateFactName, name, attributeName, Double.parseDouble(from),
