...
public class JessHandlerUtils {
	...
	private static Set<String> buildReplaceConstantsAttRef(Rete rete, Instance owner, String n) {
		...
		String att = ParsingUtils.extractChildAttributePathWithoutParent(owner.getDefinition(), n);// ParsingUtils.resolveAttribute(n);
	...
	private static Set<String> buildReplaceConstantsInstanceRef(Rete rete, Instance owner, String n) {
		...
		String objectWithList = ParsingUtils.resolveChildFrameWithList(owner.getDefinition(), n);// ParsingUtils.resolveObjectClassNoList(n);
		...
		for (Instance inst : owner.getChildInstances(listName)) {
			...
			if (ref instanceof IntervalAttribute) {
				double val = ((IntervalAttribute) ref).getValue();
				assertParameter(rete, attributePathInChild, String.valueOf(val));
			} else {
				if (ref == null) {
					System.out.println("not resolved:" + attributePathInChild);
				}
				assertParameter(rete, objectWithList + "/" + attributePathInChild, ref.toValueString());
			}
	...
	private static Set<String> buildReplaceVariablesSC(Rete rete, Instance owner, String n) {
		...
		String listName = ParsingUtils.resolveList(n);
		String att = ParsingUtils.extractChildAttributePathWithoutParent(owner.getDefinition(), n);// ParsingUtils.resolveAttribute(n);
		String object = ParsingUtils.resolveChildFrameWithoutList(owner.getDefinition(), n);// ParsingUtils.resolveObjectClassNoList(n);
