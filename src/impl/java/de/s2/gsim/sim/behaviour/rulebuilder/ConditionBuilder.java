...
public abstract class ConditionBuilder {
	...
	/**
	 * Call this method if the operator is exist/not-exists. Then the whole condition is interpreted accordingly; for example, if the
	...
	private static String buildVariableExistQuantifiedConditionPart(Instance agent, ConditionDef cond, Object2JessVariableBindingTable refs,
		...
		if (negated) {
			s.append(" (or (not (exists (object-parameter (object-class ?pName").append(variableIndex).append("&:(eq pName")
			        .append(variableIndex).append(" \"").append(objPath).append("\")) (instance-name ").append(binding).append(") )))\n");

			s.append("  (and (object-parameter (object-class \"").append(objPath).append("\") (instance-name ").append(binding)
			        .append("))\n");

			s.append("  (parameter (name ?n0&:(eq ?n0 (str-cat \"").append(list).append("/\" ").append(binding).append(" \"/")
			        .append(remainingPath).append("\")))").append(" (value ?exparam").append(variableIndex + 1).append("))\n");

		} else {

			s.append(" (object-parameter (object-class \"").append(objPath).append("\") (instance-name ").append(binding).append("))\n");

			s.append(" (parameter (name ?n1&:(eq ?n1 (str-cat \"").append(list).append("/\" ").append(binding).append(" \"/")
			        .append(remainingPath).append("\"))) ").append(" (value ?exparam").append(variableIndex + 1).append("))\n");
		}
		return s.toString();
	}

	private static String buildConstantExistsQuantifiedConditionPart(Instance agent, ConditionDef cond,
	        Object2JessVariableBindingTable refs, String objectPath, String attPath, int variableIndex, String valueVariableBinding) {
	...
	public static String createAtomCondition(Instance owner, ConditionDef cond, Object2JessVariableBindingTable objRefs, String nRule_1)
		...
		if (!Utils.isNumerical(cond.getParameterValue()) && !isJessFunc(cond.getParameterValue()) && !cond.getOperator().equals("<>")) {
			...
			nRule += "(value ?value" + k + "&:(and (numberp ?value" + k + ") (" + cond.getOperator() + " ?value" + k + " "
			        + cond.getParameterValue() + " ))))\n";
		} else if (!Utils.isNumerical(cond.getParameterValue()) && !isJessFunc(cond.getParameterValue())
	...
	public static String createIntervalAtomCondition(Instance owner, String attributeName, double min, double max,
		...
		int variableIndexBase = cern.jet.random.Uniform.staticNextIntFromTo(0, 1000);

		nRule.append(createLHS(owner, attributeName, objRefs, variableIndexBase + 12001, nRule1));
	 ...
	 * @param objRefs table with the indices of referenced objects by the conditions in the rule
	 * @param nRule_1 the rule string so far created from the input
	...
	private static String createLHS(Instance owner, String leftVariableName, Object2JessVariableBindingTable objRefs, int variableIdx,
	        String nRule_1) throws GSimEngineException {
