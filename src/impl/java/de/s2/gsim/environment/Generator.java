...
public abstract class Generator {
	...
	private static Instance randomiseNormallyDistributedAttributeValues(Instance obj, double svar) {
		...
		for (int j = 0; j < lists.size(); j++) {
			...
			for (int k = 0; k < atts.size(); k++) {
				DomainAttribute da = obj.getDefinition().getAttribute(listName, atts.get(k).getName());
				Attribute att = atts.get(k);
                randomiseAttributeNormallyDistributed(obj, svar, listName, da, att);
            }
		}
		return obj;
	}

    public static Instance randomiseAttributeNormallyDistributed(Instance obj, double svar, String listName, DomainAttribute da, Attribute att) {
        if (da.isMutable()) {
            if (att instanceof OrderedSetAttribute) {
                OrderedSetAttribute set = (OrderedSetAttribute) att;
            ...
            } else if (att instanceof SetAttribute) {
                SetAttribute set = (SetAttribute) att;
                if (set.getFillers().size() > 0) {
                    int v = cern.jet.random.Uniform.staticNextIntFromTo(0, da.getFillers().size() - 1);
                    set.removeAllEntries();
                    set.addEntry(set.getFillers().get(v));
                }
            } else if (att instanceof IntervalAttribute) {
                ...
                if (interval > 0) {
                    ...
                    IntervalAttribute iatt = (IntervalAttribute) att;
                    iatt.setFrom(v);
                    iatt.setTo(v);
                }
            } else if (att instanceof NumericalAttribute) {
                NumericalAttribute natt = (NumericalAttribute) att;
            ...
            obj.addOrSetAttribute(listName, att);
        }
        return obj;
    }
